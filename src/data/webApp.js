export const apps = [
  {
    name: 'ColorPicker',
    description: 'ColorPick（拾色） - 用于识别相机或图像中的颜色的应用程序。',
    logo: '/static/pickcolor/logo.png',
    previewUrl: 'https://colorpicker-bpx.pages.dev/#/',
    googlePlayUrl:
      'https://play.google.com/store/apps/details?id=com.git.hui.colorpicker.google',
  },
  {
    name: 'Printo',
    description: '普染千格，智印万象 | 免费在线打印模板设计器',
    logo: '/static/printo/cover.jpg',
    googlePlayUrl:
      'https://play.google.com/store/apps/details?id=com.git.hui.printo',
    previewUrl: 'https://ai.hhui.top/printo/#/',
  },
  {
    name: 'Sleep Sound 白噪音离线版',
    description: '【沉浸式睡眠革命】告别失眠焦虑！用科学声波重构你的深度睡眠',
    logo: '/static/babymusic/logo.webp',
    googlePlayUrl:
      'https://play.google.com/store/apps/details?id=com.git.hui.babymusic.google',
    // previewUrl: "https://ai.hhui.top/imgline/index.html",
  },
  {
    name: '线稿提取器',
    description: '免费在线的图片转线稿工具',
    logo: '/static/webapp/linedraw.jpg',
    googlePlayUrl: '',
    previewUrl: 'https://ai.hhui.top/app/imgline.html',
  },
  {
    name: 'QuickTools',
    description:
      '神奇的百宝箱，收集各路直接在本地浏览器实现全功能的小工具，安全无风险，支持离线使用',
    logo: 'https://tool.hhui.top/icon.svg',
    googlePlayUrl: '',
    previewUrl: 'https://tool.hhui.top/',
  },
  {
    name: '像素块填色',
    description: '在线像素块填色游戏',
    logo: 'https://tool.hhui.top/icon.svg',
    googlePlayUrl: '',
    previewUrl: 'https://liuyueyi.github.io/ai-web-case/',
  },
  {
    name: '收入实时计算器',
    description: '在线实时显示收入增长',
    logo: '/app/static/earn.webp',
    googlePlayUrl: '',
    previewUrl: '/app/earn.html',
  },
];
